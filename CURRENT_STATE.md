...
| Active package | `Package 3AK — Import Insights Registry-Driven Dispatcher Consolidation` — IN PROGRESS on `feature/import-insights-registry`; `renderImportInsights` now iterates the ordered `IMPORT_INSIGHT_RENDERERS` registry; `window.__km` bridge block left literal; behavior-preserving; implementation + docs done, verification gate green, stop-before-commit awaiting Coordinator commit authorization |
