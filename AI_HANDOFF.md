...
**Next recommended action:** Await Coordinator authorization for **Package P4 — Group-chat correctness** (group vs 1:1; per-speaker identity retained — no "them" collapse; system events surfaced), and/or **sanitized WhatsApp iOS fixture gathering** for real-world validation (1:1 + group, with/without media ZIP, incl. the Abena/N regression case). **P4 has not started.** Do not begin P4 or any further implementation without explicit Coordinator authorization.
...
| **Active pass** | None |
| **Active branch** | `main` |
| **base HEAD** | `30f0733` (feat(import): add WhatsApp self-identification and participant mapping to canonical path — Package P3; `main` HEAD — this state-sync adds one docs commit on top) |
| **Active package** | None |
...
| **Prior completed pass** | Package P2 — WhatsApp Text Parser Hardening (engine/parser only, behavior-preserving) — impl `8d7ed86`, fast-forward merged to `main` 2026-06-16 (P1 — Canonical Import Model + Adapter Contract, impl `bd57c8a`) |
