...
| **Active pass** | None |
| **Active branch** | `main` |
| **base HEAD** | `8c80627` (feat(import): native WhatsApp ZIP reader and media manifest resolution; `main` HEAD — this state-sync adds one docs commit on top) |
| **Active package** | None |
| **Last completed work** | Package P5A — Native ZIP Reader Spike + Dependency Decision Gate (engine/test only; zero-dependency `KMEngine.WhatsAppZip` + adapter `opts.mediaManifest`/`importZip`) — impl `8c80627`, fast-forward merged to `main` 2026-06-17 |
