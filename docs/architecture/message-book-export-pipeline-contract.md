...
| `render-environment-known` | `MessageBookRenderEnvironment` (8G) — decomposes cover / spine / safe-area / bleed-trim / parity / font / interior / export-target | **honestly false** — geometry/parity/interior/export-target are known, but cover, spine width (vendor-supplied paper/board thickness), and font/emoji render availability are genuinely missing. See `message-book-render-environment-contract.md`. |
...
- **Live input mapping (existing live facts only)** — `renderBookExportPreflightStatus(...)` calls `MessageBookExportPipeline.resolveFromContext` with:
  ...
  - `renderEnvironmentKnown` — the **8G** `MessageBookRenderEnvironment` aggregate, derived (since 8G) from live repo truth (`BOOK_PRODUCTION_DEPS` / `BOOK_PARITY` geometry + the captured production-dependency flags) instead of a hardcoded literal. Trim/bleed/safe-area/parity/export-target/interior are known; cover, spine width, and font/emoji render availability are genuinely missing, so the aggregate is **honestly `false`** — the value is now computed by the render-environment contract rather than invented, and a future package that confirms those inputs advances it automatically. See `message-book-render-environment-contract.md`.
