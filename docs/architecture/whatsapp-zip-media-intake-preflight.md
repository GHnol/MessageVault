...
## 11. P5B consolidation + edge-case hardening result (2026-06-17)

**Status:** P5B IMPLEMENTED on `feature/whatsapp-data-foundation-p5b-zip-intake-hardening` (base `main` @ `5d1972d`) — **engine/test only, zero dependencies, no `package.json`, no `index.html`, no UI, no `scripts/**`, no fflate, no real ZIP/media committed** — awaiting Coordinator commit authorization. This is the §6 "P5B (+P5C)" engine row: consolidate the P5A spike into a production-safe intake and harden every documented edge case.

**What changed (engine, both files were already present from P5A):**
- `src/core/whatsapp-zip-reader.js` — `buildMediaManifest` now distinguishes two collision classes: an **exact-duplicate archive entry** (`DUPLICATE_ARCHIVE_ENTRY`, the same archive-relative name twice) from a **duplicate media basename** (`DUPLICATE_MEDIA_BASENAME`, the same basename under different paths) and flags the kept manifest entry `ambiguous:true`; **absolute / `..`-traversal entry names** are diagnosed (`SUSPICIOUS_ENTRY_NAME`) so `sourceRef` can never become an absolute path silently. New exported pure helper `isSuspiciousName`.
- `src/adapters/whatsapp-txt-adapter.js` — the manifest resolver raises **`AMBIGUOUS_MEDIA_MATCH`** when an `<attached: NAME>` marker binds to an ambiguous basename (it still resolves the first occurrence deterministically — never a silent guess) and **`INVALID_MEDIA_MANIFEST`** when `opts.mediaManifest` is non-null but not an array (attachments left `present:null`, no fabricated misses). `indexManifest` detects in-manifest basename ambiguity directly, so a directly-supplied manifest is covered as well as one produced by `KMEngine.WhatsAppZip`.
- `src/core/canonical-conversation.js` and `src/core/import-adapter-contract.js` remain **unchanged** (model already ZIP-ready; all P5B signal rides in `diagnostics.warnings` / `mediaMissing`).

**Confirmed by new synthetic tests (no committed binary):** media bytes are **never decompressed** (manifest builds correctly with corrupt/garbage deflate media bytes and with no `DecompressionStream` present, taking `byteSize` from the central directory); only `_chat.txt` is decompressed; malformed / truncated / corrupt-central-directory / cdOffset-past-EOF archives are handled safely **without throwing**; the **decompression-unavailable** path surfaces `DECOMPRESSION_UNAVAILABLE` end-to-end (stored chat still extracts); **every `importZip` failure path returns a contract-valid empty conversation and never throws** on odd inputs; `<Media omitted>` stays `present:false`/`omitted` and is never confused with missing archive media; `sourceRef` stays archive-relative-only with a deterministic manifest. Tests: `whatsapp-zip-reader-tests.mjs` 62→**117**, `whatsapp-txt-adapter-tests.mjs` 305→**316**, `km-engine-tests.mjs` 211→**217**. Node 4068→**4140 / 32 suites**; E2E 57/57 + 195/195; VR Scenario A 4/4 — all green and unchanged (engine/test only).

**Dependency decision (unchanged, reconfirmed):** native no-dependency reading remains sufficient across the full hardened synthetic envelope. **Continue native-only; do not add fflate.** ZIP64 / non-UTF-8 archives stay loudly rejected. The single residual unknown is still **real archive shape** — native correctness is proven against synthetic fixtures but not yet a sanitized real `.zip`; gather sanitized 1:1 + group with-media exports (incl. the Abena/N case) into the gitignored `scripts/fixtures/private/` for a future fixture-gated validation. fflate remains a Coordinator-gated fallback only if a real archive later exposes ZIP64 / exotic-encoding cases.

---

## 12. P5C private ZIP validation harness (2026-06-17)

**Status:** P5C IMPLEMENTED on `feature/whatsapp-data-foundation-p5c-private-zip-validation` (base `main` @ `3029c57`) — **local validation tooling + tests + docs only; no `index.html`, no UI, no app `src/**` behavior change, no `package.json`/dependency, no fflate, no real ZIP/media committed** — awaiting Coordinator commit authorization. This package builds the mechanism that closes the **"real archive shape" residual unknown** §10/§11 keep flagging: a safe way to validate real/sanitized exports locally without committing or exposing any private data.

**What was built:**
- `scripts/validate-private-whatsapp-zips.mjs` — scans the gitignored `scripts/fixtures/private/whatsapp/` directory and validates each `.zip` against the **production native path** (`KMEngine.WhatsAppZip.readCentralDirectory`/`readArchive` + `whatsappTxtAdapter.importZip` + `ImportAdapterContract.validateConversation`), loaded via the same vm-module pattern the unit tests use (so it exercises the real reader, not a copy). It emits a **privacy-safe summary** — file/archive/message/participant/system-event/media counts, present/missing/omitted media counts, compression-method counts, name-encoding counts, ZIP64/encrypted/unsupported-method findings, contract validity, and diagnostics **tallied by code only** — and classifies each archive **PASS / WARN / FAIL**. It never prints message text, names, phone numbers, or filenames; a structural `redactionSelfCheck` aborts before any output if a non-enum value would appear. With no fixtures it prints `SKIP — NO_PRIVATE_FIXTURES` and exits 0. `--selftest` proves the pipeline on synthetic in-memory archives; `--json` is privacy-safe machine output; `--strict` makes WARN non-zero; `--debug` (opt-in, off by default, loudly flagged "do not paste") is the only mode that may surface raw names.
- `docs/qa/private-whatsapp-zip-validation.md` — placement, sanitization (preserve structure / `_chat.txt` / `<attached:>` markers / compression layout; replace bodies, names, phones, and media with same-named dummies), what is and is not safe to share, PASS/WARN/FAIL interpretation, the fixtures to gather (1:1 + group with-media, Abena/N, non-US locale), and the P5D gate.
- `src/tests/whatsapp-zip-reader-tests.mjs` — Suite 16 (117→**135**) imports the harness summarizers and asserts privacy-safety + PASS/WARN/FAIL classification + that the self-check catches name/filename-shaped fields. `canonical-conversation.js` / `import-adapter-contract.js` / `whatsapp-zip-reader.js` / `whatsapp-txt-adapter.js` engine logic is **unchanged** (the only `whatsapp-zip-reader.js`-adjacent edit is the new test suite + script). Node 4140→**4158 / 32 suites**; E2E 57/57 + 195/195; VR Scenario A 4/4 — all green and unchanged.

**Real-ZIP validation gate (the point of P5C):** P5D (the `.zip` ingest UI in `index.html`) stays blocked until at least one **real sanitized with-media** archive — ideally both a 1:1 and a group export — reaches **PASS** through this harness, or **WARN** whose diagnostics are understood and accepted. A genuine WhatsApp export that **FAILs** (esp. ZIP64 or non-UTF-8/CP437 names) is the trigger to return the dependency decision to the Coordinator (vendored/pinned fflate fallback) **before** P5D, not after. Until real fixtures are validated, the native-only decision remains "proven against synthetic, pending real archive shape."

---

## 13. P6 import-diagnostics + coverage consolidation (2026-06-17)

**Status:** P6 IMPLEMENTED on `feature/whatsapp-data-foundation-p6-import-diagnostics-consolidation` (base `main` @ `87c0c66`) — **diagnostics + coverage + docs + small UI status consistency only; no new product feature, no engine logic change, no `package.json`/dependency, no fflate, no real ZIP/media/private fixtures committed** — awaiting Coordinator commit authorization. This is the §5 "P6 — import diagnostics + test-coverage consolidation" row: make the diagnostic vocabulary internally consistent across every import path before returning to product-facing Message Book work.

**What was consolidated:**
- **One canonical diagnostic vocabulary**, documented in `docs/qa/private-whatsapp-zip-validation.md` §11: fatal `reason` codes (archive/`_chat.txt` cannot be read → `ZIP_READ_FAILED` + plain-language browser error + harness FAIL) vs non-fatal notice codes (`ok:true`, surfaced in the `#zipImportStatus` panel + harness WARN), the `importZip` wrapper codes (`ZIP_READ_FAILED`/`ZIP_READER_UNAVAILABLE`), and harness PASS/WARN/FAIL. The vocabulary is now spoken consistently by the reader, the adapter, the harness, and the browser.
- **`UNSUPPORTED_COMPRESSION` dual-nature clarified and locked:** fatal when it is the `_chat.txt` entry's method (the chat cannot be read), non-fatal when it is a media entry's method (media is manifest-only and never decompressed).
- **Browser fatal-error map fixed (`index.html` `zipFailureMessage`):** added a plain-language `UNSUPPORTED_COMPRESSION` case (the chat-file-unsupported path previously fell through to the default branch, which interpolated the raw enum code into the user message); removed the dead `TRUNCATED_CENTRAL_DIRECTORY` case (a non-fatal notice — `readCentralDirectory` returns `ok:true` — never a fatal `archive.reason`). No other `index.html` behavior changed; the `#zipImportStatus` panel still shows enum-coded notices (privacy-safe counts + codes only).
...
**Coverage (single new suite):** `src/tests/whatsapp-zip-reader-tests.mjs` Suite 17 (12 tests, 135→**147**) locks the vocabulary: `readArchive`/`importZip` surface `UNSUPPORTED_COMPRESSION` as a fatal reason for an unsupported-method `_chat.txt` (and `importZip` wraps it as `ZIP_READ_FAILED`, contract-valid, empty), an unsupported-method media entry stays a non-fatal diagnostic with the manifest still built, `TRUNCATED_CENTRAL_DIRECTORY` is a non-fatal notice never surfaced as a fatal `reason`, `REVIEW_CODES` matches the documented WARN vocabulary exactly, and every observed fatal reason is a documented `UPPER_SNAKE` enum. `whatsapp-txt-adapter-tests.mjs` and `km-engine-tests.mjs` are unchanged (existing suites already cover those layers — no genuine gap). `src/core/whatsapp-zip-reader.js`, `src/adapters/whatsapp-txt-adapter.js`, `src/core/canonical-conversation.js`, and `src/core/import-adapter-contract.js` engine logic is **unchanged**. Node 4158→**4170 / 32 suites**; E2E 57/57 + 195/195; VR Scenario A 4/4 + import-panels 10/10 — all green and unchanged.

**Dependency decision (unchanged, reconfirmed):** native no-dependency reading remains the path; **do not add fflate**. **Real-world ZIP import remains fixture-gated** — P6 consolidates and locks the *synthetic* envelope and documents the vocabulary, but it does not validate any real sanitized with-media archive. The residual unknown is still real archive shape; the next step remains gathering sanitized 1:1 + group with-media exports (incl. the Abena/N case) into the gitignored `scripts/fixtures/private/whatsapp/` and running `node scripts/validate-private-whatsapp-zips.mjs`.
