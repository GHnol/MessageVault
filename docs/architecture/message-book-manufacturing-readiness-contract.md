...
## 8F — Live Export Preflight Status Hook + Dogfood Gate

**Status:** Active — introduced in Message Book Manufacturing Readiness 8F. **Read-only visibility / dogfood package — `index.html` + tests + docs only, no production behavior added.** 8F surfaces the 8E `MessageBookExportPipeline` contract in the live app as a read-only `#bookExportPreflightStatus` panel (a sibling of the 8B `#bookManufacturingStatus` / 8D `#bookPrintSpecPanel`, **outside `#bookCanvas`**), so a user can see which export inputs are known vs missing and that artifact generation is not implemented. Authoritative 8F detail: `docs/architecture/message-book-export-pipeline-contract.md` § "Live status hook (8F)".

**This engine (8A) is unchanged by 8F.** 8F reads the 8E contract directly (`resolveFromContext`) from already-decided live facts (the 8D print-spec result + proof/composition signals; render-environment honestly reported missing) and renders the resulting `describeReadiness` view-model read-only. It registers no buttons, generates no print/export/PDF/file/vendor packet, selects no vendor, never flips `exportPipelineImplemented`, never marks `print-file-ready`/vendor/manufacturing/packaging true, and adds no checkout/payment/cart/order/order-submission/address/shipping/tax/price/SKU/line-item behavior. The 8B live `#bookManufacturingStatus` path, the 8A `CAPABILITIES`/`LOCAL_INTENT_REQUIRED`, and 8C/8D/7A–7E/5D/5E/6A/6B/6C are all intact. WhatsApp P1–P6 and the native no-dependency ZIP path are unchanged; no dependency, no `package.json`, no import/WhatsApp/ZIP change. Coverage: `message-book-export-pipeline-tests.mjs` 466 → **589** (Suite 20 — live `renderBookExportPreflightStatus` input mapping + dogfood gate); no change to `message-book-manufacturing-readiness-tests.mjs` (348).
