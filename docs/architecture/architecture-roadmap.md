...
**Last updated:** 2026-06-08 (Package 3AK — Import Insights Registry-Driven Dispatcher Consolidation — COMPLETE, impl/merge `052346f`, fast-forward merged to `main` 2026-06-08; behavior-preserving — `renderImportInsights` now iterates the ordered `IMPORT_INSIGHT_RENDERERS` registry; no new engine, no new panel, no DOM/CSS/order/copy/visibility/behavior change; baseline unchanged 3645 / 30 / 57 / 195 / VR PASS. Prior: Package 3AJ — Import Insights Consolidation — COMPLETE, impl/merge `92435b7`; `renderImportInsights` dispatcher; no new engine, no new panel, no behavior change. Package 3AH — Reaction Analysis Engine + Panel — COMPLETE — impl `a165122`, merged to `main`)
**Status:** Active
...
src/
  core/
    ...
    message-length-analysis.js  — KMEngine.MessageLengthAnalysis; compute(memories) pure function; returns { avgCharsPerMessage: number, longestMessage: { sender, length } | null, perSenderStats: [{ sender, avgCharsPerMessage, messageCount }] }; skips senderRole:system, isAttachmentOnly:true, type:attachment-placeholder, non-string/blank text; avgCharsPerMessage rounded to 1 decimal; longestMessage tie-break: earliest valid occurrence; perSenderStats sorted avgChars desc then name asc; zero-state for empty/invalid/no-valid-message input; pure, no DOM, no side effects — Package 3AE
    conversation-initiation.js  — KMEngine.ConversationInitiation; compute(memories) pure function; returns { totalConversations: number, topInitiator: { sender, initiationCount } | null, perSenderStats: [{ sender, initiationCount, initiationPct }] }; filters non-system messages with valid timestamps, sorts ascending; a conversation start = first valid message + any message whose gap from the previous valid message >= GAP_THRESHOLD_MS (named constant, 6 hours); topInitiator = highest initiationCount, tie-break sender asc; perSenderStats sorted initiationCount desc then name asc; initiationPct = count/total × 100 rounded 1 decimal; zero-state for empty/invalid/no-valid-message input; pure, no DOM, no side effects — Package 3AF
    reaction-analysis.js        — KMEngine.ReactionAnalysis; compute(memories) pure function; returns { totalReactions, messagesWithReactions, topReactionEmojis: [{ emoji, count, rank }], topReactor: { reactor, count } | null, mostReactedToSender: { sender, count } | null }; reads NormalizedMemory.reactions[] (captured Package 3AG); MAX_TOP=5 for emojis; counts by emoji, by reactor, and by reacted-to message sender; skips null/empty emoji and reactor for their tallies; tie-break count desc then string asc; zero-state for empty/invalid/no-reaction; pure, no DOM, no side effects — Package 3AH
    ...
    canonical-conversation.js — KMEngine.CanonicalConversation; pure builders for Conversation, Participant, Message, MediaAttachment, Reaction, Reply, SystemEvent, SourceMetadata, ImportDiagnostics + groupMessages (derived view); deterministic IDs; engine-only, not yet wired into index.html — Package P1 (canonical import-truth model)
    import-adapter-contract.js — KMEngine.ImportAdapterContract; validateConversation (canonical output shape) + validateAdapter (adapter interface); semantic guard rejects commerce/readiness fields; future WhatsApp/iMessage/Meta/Telegram adapters target this — Package P1
  ...
  adapters/
    ...
    instagram-dm-adapter.js        — KMEngine.instagramDmAdapter; Instagram DM JSON export; HTML entity decoding; media+share → attachment-placeholder; senderRole always contact; reaction capture (Meta { reaction, actor } → canonical { reactor, emoji, label }) — Package 3AG; ADAPTER_ID instagram-dm-json-v1; browser-loaded (Package 3P) — Package 3O/3P
...
index.html (conversation stats panel) — #conversationStatsPanel; renderConversationStatsPanel(memories); indigo tone; chips: busiestDay, longestStreak, avg messages/day, top sender; called at all 11 import/open sites; window.__km.renderConversationStatsPanel — Package 3Y
    facebook-messenger-adapter.js  — KMEngine.facebookMessengerAdapter; Facebook Messenger JSON export; magic_words discriminator (present in FB, absent in Instagram DM); HTML entity decoding; media+share → attachment-placeholder; senderRole always contact; reaction capture with mojibake decode (Meta { reaction, actor } → canonical { reactor, emoji, label }) — Package 3AG; ADAPTER_ID facebook-messenger-json-v1; browser-loaded (Package 3S) — Package 3R/3S
...
index.html (enterComposition)        — ProductDraft lifecycle wiring: initDraft + advanceDraft none→in-progress on message-book entry; getGroupDraft test helper on window.__km — Package 3G
  tests/
    ...
    facebook-messenger-adapter-tests.mjs   — 113 tests; API shape, canHandle (accepts/rejects/magic_words discriminator), fixture rawCounts, timestamp conversion, HTML entity decoding, senderRole, text/attachment normalization, NormalizedMemory fields, importWarnings, no-throw, participants, semantic guards — Package 3R; reaction capture (Suite 18: mojibake decode + fallback-preserve, malformed-safe, IQR counts) — Package 3AG
    instagram-dm-adapter-tests.mjs          — 101 tests; API shape, canHandle accepts/rejects, fixture rawCounts, timestamp conversion, HTML entity decoding (sender+content), senderRole, text/attachment normalization, NormalizedMemory fields, importWarnings, no-throw, semantic guards, participants — Package 3O; reaction capture (Suite 16: clean-unicode, malformed-safe, IQR counts) — Package 3AG
    ...
    message-length-analysis-tests.mjs     — 82 tests; API shape, empty/null/non-array zero-state, system-message exclusion, attachment-only/attachment-placeholder exclusion, blank/non-string text skip, avgCharsPerMessage accuracy (1-decimal rounding), longestMessage accuracy, longestMessage tie-break (earliest valid occurrence), perSenderStats accuracy/sort (desc avg then alpha), skip rules combined, no-throw, semantic guards — Package 3AE
    conversation-initiation-tests.mjs     — 90 tests; API shape, empty/null/non-array zero-state, no-valid-message zero-state, system-message exclusion, invalid/missing/null/falsy timestamp skip, single-message conversation, first valid message as start, gap below/equal/above threshold, multiple starts across sorted timestamps, input-order independence, topInitiator accuracy/tie-break, perSenderStats count accuracy, initiationPct 1-decimal rounding, sort (count desc then alpha), no-throw, fixture behavior, semantic guards — Package 3AF
    reaction-analysis-tests.mjs            — 66 tests; API shape, empty/null/non-array zero-state, no-reactions zero-state, totalReactions, messagesWithReactions, topReactionEmojis sort/rank/MAX_TOP/tie-break, topReactor accuracy/tie-break, mostReactedToSender accuracy/tie-break, malformed reactions[] no-throw, null/empty emoji + reactor handling, fixture behavior (mirrors fake-instagram-dm.json), ImportQualityReport preservation regression, semantic guards — Package 3AH
...
DELIVERED (Package 3AK — Import Insights Registry-Driven Dispatcher Consolidation, impl/merge `052346f`, fast-forward merged to `main` 2026-06-08):
- `index.html` — `renderImportInsights(memories)` is now registry-driven: an ordered `IMPORT_INSIGHT_RENDERERS` array lists the ten existing panel renderers in their exact current order (`renderImportQualityPanel` → `renderReactionAnalysisPanel`), and the dispatcher iterates that array. The ten `renderXPanel` functions, their individual `window.__km` bridges, the literal `window.__km` bridge block, `window.__km.renderImportInsights`, and all 11 dispatcher call sites are preserved unchanged. No DOM/CSS/panel-order/panel-copy/visibility/behavior change — pure wiring consolidation completing the Package 3AJ debt-paydown; no new engine, no new panel. Per the Package 3AK scope, the `window.__km` bridge block is deliberately NOT generated from the registry (left literal to avoid key-name drift). Baseline unchanged: 3645 Node / 30 suites; 57 seeded; 195 real-files; visual regression PASS.
...
- `src/tests/km-engine-tests.mjs` — loads `conversation-initiation.js`; `ConversationInitiation — smoke` suite added (+6 → 174 total).
- `index.html` — CSS for `.conversation-initiation-panel` + `.conversation-initiation-inner` + `.conversation-initiation-chip` (pink/magenta tone: light `#fce4ec`/`#880e4f`, dark `#1a0014`/`#f8bbd0`); dark mode overrides; `<script src="src/core/conversation-initiation.js">` tag after message-length-analysis.js; `<div id="conversationInitiationPanel">` after `#messageLengthPanel`; `const conversationInitiationPanel` binding; `renderConversationInitiationPanel(memories)` function; chips: totalConversations, topInitiator, top initiator initiationPct; calls at all 11 import/open sites; `window.__km.renderConversationInitiationPanel` exposed.
- `scripts/e2e-regression-harness.mjs` — `CI_FIXTURE` + `CI_FIXTURE_COUNT = 12` constants; Phase 43 (6 real-files tests): hidden before import → visible after CI fixture import → count=12 → "Alice" top initiator chip → conversation count/percentage chip → TXT reimport resets state for Phase 12.
- `docs/qa/test-strategy.md` — Phase 43 note; real-files baseline 183→189; Node baseline 3448→3544 (29 suites); corrected stale pre-commit baseline list.
- `docs/architecture/architecture-roadmap.md` — this file; Package 3AF entry.

DELIVERED (Package 3AG — Meta Reaction Capture, 2026-06-08):
- `src/adapters/instagram-dm-adapter.js` + `src/adapters/facebook-messenger-adapter.js` — map `msg.reactions` (Meta `{ reaction, actor }`) into `NormalizedMemory.reactions[]` as canonical `{ reactor, emoji, label }`, replacing the hardcoded `reactions: []`. New per-adapter `mapReactions()` + `decodeReaction()` helpers: `decodeReaction()` repairs Latin-1-escaped-UTF-8 mojibake via `decodeURIComponent(escape())` and preserves the raw string on incomplete/failed decode (never drops); `mapReactions()` is non-array/malformed-safe and skips entries lacking both `reaction` and `actor`. No change to message IDs, sender/timestamp/text/type normalization, rawCounts, skip behavior, or `generateMemoryId`. `ImportQualityReport.totalReactionCount` / `messagesWithReactionsCount` become real for Meta imports automatically.
- `scripts/fixtures/fake-instagram-dm.json` (2 clean-unicode reactions) and `scripts/fixtures/fake-facebook-messenger.json` (1 mojibake → 👍 plus 1 clean) enriched; message counts unchanged (8 imported each).
...
- `index.html` — CSS for `.reaction-analysis-panel` + `.reaction-analysis-inner` + `.reaction-analysis-chip` (rose/crimson tone: light `#ffebee`/`#b71c1c`, dark `#1a0006`/`#ef9a9a`); dark mode overrides; `<script src="src/core/reaction-analysis.js">` tag after conversation-initiation.js; `<div id="reactionAnalysisPanel">` after `#conversationInitiationPanel`; `const reactionAnalysisPanel` binding; `renderReactionAnalysisPanel(memories)` function; chips: totalReactions, top emoji, topReactor, mostReactedToSender; hidden when totalReactions === 0; called at all 11 import/open sites; `window.__km.renderReactionAnalysisPanel` exposed.
- `scripts/e2e-regression-harness.mjs` — Phase 44 (6 real-files tests) reusing `fake-instagram-dm.json` (2 reactions; Alice Smith top reactor; bob_jones_99 most reacted-to); Phase 43's reset test relabeled to feed Phase 44.
- `docs/qa/test-strategy.md` — Phase 44 note; real-files baseline 189→195; Node baseline 3573→3645 (30 suites).
- `docs/architecture/architecture-roadmap.md` — this file; reaction-analysis.js module map + `#reactionAnalysisPanel` panel + reaction-analysis-tests.mjs + Package 3AH entry.
- No adapter changes; no `import-quality-report.js` / `normalized-memory.js` changes; no DEF-11 in-book reaction rendering; no scope-guarded files touched.
...
- `src/tests/km-engine-tests.mjs` — loads `message-length-analysis.js`; `MessageLengthAnalysis — smoke` suite added (+6 → 168 total).
- `index.html` — CSS for `.message-length-panel` + `.message-length-inner` + `.message-length-chip` (cyan/sky-blue tone: light `#e0f7fa`/`#006064`, dark `#001a1a`/`#b2ebf2`); dark mode overrides; `<script src="src/core/message-length-analysis.js">` tag after response-time-analysis.js; `<div id="messageLengthPanel">` after `#responseTimePanel`; `const messageLengthPanel` binding; `renderMessageLengthPanel(memories)` function; chips: avgCharsPerMessage, longestMessage.sender; calls at all 11 import/open sites; `window.__km.renderMessageLengthPanel` exposed.
- `scripts/e2e-regression-harness.mjs` — `ML_FIXTURE` + `ML_FIXTURE_COUNT = 12` constants; Phase 42 (6 real-files tests): hidden before import → visible after ML fixture import → count=12 → avg chars chip → "Alice" longest message sender chip → TXT reimport resets state for Phase 12.
- `docs/qa/test-strategy.md` — Phase 42 note; real-files baseline 177→183; Node baseline 3360→3448 (28 suites).
- `docs/architecture/architecture-roadmap.md` — this file; Package 3AE entry.
