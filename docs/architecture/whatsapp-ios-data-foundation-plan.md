...
**Implementation progress (updated 2026-06-16):** **P1 COMPLETE** (canonical model + adapter contract — impl `bd57c8a`, merged to `main`). **P2 COMPLETE** (WhatsApp text parser hardening — impl `8d7ed86`, merged to `main`; `whatsappTxtAdapter.toCanonical(rawText, opts)` hardened parse path). **P3 IN PROGRESS** (self-identification + participant mapping — branch `feature/whatsapp-data-foundation-p3-self-id-participants`; `opts.self` sets participant-level `isSelf`; engine-only, no UI; awaiting commit authorization). See the package-breakdown table in §5 for per-package status; `AI_HANDOFF.md` / `CURRENT_STATE.md` carry the live state.
...
| **P3** *(was P4)* 🔄 IN PROGRESS | Self-identification + participant mapping | engine-only (no UI this package) | Participant roster with stable ids; **self at participant level** via `opts.self` (exact / normalized / alias / phone-like / participant-id match); only the unique match flips `isSelf`; ambiguous/no-match/invalid recorded in `ImportDiagnostics`; the **one-sided-sender regression test** landed. Branch `feature/whatsapp-data-foundation-p3-self-id-participants`; Node 3890→3938; awaiting commit authorization. |
...
| **P6** | Import diagnostics + test-coverage consolidation | engine + tests | Structured ImportDiagnostics surfaced; consolidate malformed/locale/privacy tests; E2E phases. |
