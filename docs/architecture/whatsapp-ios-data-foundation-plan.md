...
**Implementation progress (updated 2026-06-16):** **P1 COMPLETE** (canonical model + adapter contract — impl `bd57c8a`). **P2 COMPLETE** (WhatsApp text parser hardening — impl `8d7ed86`). **P3 COMPLETE** (self-identification + participant mapping — impl `30f0733`; `opts.self` sets participant-level `isSelf`). **P4 IN PROGRESS** (group-chat correctness — branch `feature/whatsapp-data-foundation-p4-group-chat-correctness`; evidence-based group detection, title/roster inference, typed system events; engine-only, no UI; awaiting commit authorization). All merged to `main`. See the package-breakdown table in §5 for per-package status; `AI_HANDOFF.md` / `CURRENT_STATE.md` carry the live state.
