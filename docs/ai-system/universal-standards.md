...
| Custom slash commands (`.claude/commands/*.md`) | User-invoked — 16 command wrappers committed; each delegates to matching skill. Commands: `/start`, `/handoff`, `/precommit`, `/closeout`, `/package-start`, `/switch-to-codex`, `/switch-to-claude`, `/weekly-sync`, `/calendar-sync-plan`, `/status-summary`, `/os-audit`, `/project-sync-dry-run`, `/project-sync-apply`, `/notification-setup-wizard`, `/github-project-setup`, `/github-project-template`. No automatic execution; approval boundaries unchanged. |
| Project skills (`.claude/skills/*/SKILL.md`) | User-invoked — 15 skills with YAML frontmatter committed; skills are the canonical protocol layer; commands are compatibility wrappers. Skills: `start`, `handoff`, `precommit`, `closeout`, `package-start`, `switch-to-codex`, `switch-to-claude`, `weekly-sync`, `status-summary`, `os-audit`, `project-sync-dry-run`, `project-sync-apply`, `notification-setup-wizard`, `github-project-setup`, `github-project-template`. |
