...
| Field | Value |
|---|---|
| Phase | **Phase 0 rebuild; Design Bible v1 APPROVED; WhatsApp iOS data-foundation vertical underway — Package P1 + P2 + P3 COMPLETE; Package P4 (WhatsApp Group-Chat Correctness) COMPLETE (2026-06-16)** (analytics-package series CLOSED/COMPLETE through Package 3AM) |
| Active package | None |
| Active pass | None |
| Branch | `main` |
| Expected HEAD | `5a0217b` (Package P4; this state-sync adds one docs commit on top) |
...
| Last completed | **Package P4 — WhatsApp Group-Chat Correctness** (engine/parser/model only, behavior-preserving) — impl `5a0217b`, merged to `main` 2026-06-16; baseline 3980 Node / 31 suites. `toCanonical` gains evidence-based group detection (multi-speaker / create / add-remove / leave / subject / icon; `opts.isGroup` override; `WEAK_GROUP_EVIDENCE`), 1:1-stays-1:1, title/subject inference, typed SystemEvents linked to `conversationId` with extracted actors + `rosterEvidence`, distinct participants (no "them" collapse); legacy path untouched |
| Prior completed | **Package P3 — WhatsApp Self-ID + Participant Mapping** — impl `30f0733`; **Package P2 — WhatsApp Text Parser Hardening** — impl `8d7ed86`; **Package P1 — Canonical Import Model + Adapter Contract** — impl `bd57c8a`; all merged to `main` 2026-06-16 |
| Next action | Await Coordinator authorization for **Package P5 — WhatsApp ZIP / media intake** (`<attached:>` linking; MediaAttachment populated; present/missing; local-first in-browser ZIP read — starts with a dependency-evaluation sub-step) and/or sanitized WhatsApp iOS fixture gathering for real-world validation (**P5 has not started**); (A) design-system tokenization / component contracts remains an alternative. **NOT raw UI implementation.** |
