...
| Field | Value |
|---|---|
| Phase | **Phase 0 rebuild; Design Bible v1 APPROVED; WhatsApp iOS data-foundation vertical underway — Package P1 + P2 COMPLETE; Package P3 (WhatsApp Self-ID + Participant Mapping) COMPLETE (2026-06-16)** (analytics-package series CLOSED/COMPLETE through Package 3AM) |
| Active package | None |
| Active pass | None |
| Branch | `main` |
| Expected HEAD | `30f0733` (Package P3; this state-sync adds one docs commit on top) |
...
| Last completed | **Package P3 — WhatsApp Self-ID + Participant Mapping** (engine/parser only, behavior-preserving) — impl `30f0733`, merged to `main` 2026-06-16; baseline 3938 Node / 31 suites. `opts.self` on `toCanonical` sets participant-level `isSelf` (match by id / exact / normalized / alias / phone-like; ambiguous/no-match/invalid → diagnostics); non-self speakers stay distinct; one-sided-sender regression landed; legacy path untouched |
| Prior completed | **Package P2 — WhatsApp Text Parser Hardening** — impl `8d7ed86`; **Package P1 — Canonical Import Model + Adapter Contract** — impl `bd57c8a`; both merged to `main` 2026-06-16 |
| Next action | Await Coordinator authorization for **Package P4 — Group-chat correctness** (group vs 1:1; per-speaker identity retained — no "them" collapse; system events surfaced) and/or sanitized WhatsApp iOS fixture gathering for real-world validation (**P4 has not started**); (A) design-system tokenization / component contracts remains an alternative. **NOT raw UI implementation.** |
