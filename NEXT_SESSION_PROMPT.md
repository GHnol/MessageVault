...
> You are resuming work on **KeepMees / MessageVault**. The project is in **Phase 0 foundation rebuild planning**, and the **KeepMees Design Bible v1 is APPROVED (2026-06-16)** as the creative source of truth (`docs/design/keepmees-design-bible.md`). The analytics-package series (through **Package 3AM**) is **CLOSED/COMPLETE and merged to `main`**; a read-only dogfood audit of the `index.html` MVP surfaced foundational defects, so the project **pivoted from patching the MVP to rebuilding on a credible foundation**. The design-tool **taste trial is complete — NO tool was selected as creative source of truth** (Figma Make 41/70, Subframe 37/70, Onlook skipped); art direction is **human-originated via the Design Bible**; Figma/Subframe are secondary L1/L2 executors only, Onlook deferred; generic AI-generated design is rejected as the foundation (record: `docs/design/taste-trial-result.md`). **No active package. No active pass.** Branch: `main`. Expected HEAD: `756df7b` (pre-sync base; the Design Bible v1 checkpoint adds one docs commit on top). **Design Bible spine:** Quiet Monument (modern editorial), warmed by the Keepsake Letter, disciplined by the Memoir; warm bone/ivory base, single **oxblood** accent, foil ceremony only; **edited-editorial-transcript** rendering (no chat bubbles / no platform trade dress); Fraunces display + Newsreader body; Message Book = same world + ceremony. **Do not trust any memory of prior conversation.** Read, in this order: `AGENTS.md`, `CLAUDE.md` (if you are Claude Code), `AI_HANDOFF.md`, `CURRENT_STATE.md`, `docs/design/keepmees-design-bible.md`, `docs/architecture/phase-0-rebuild-decisions.md`, `docs/ai-system/README.md`, `docs/dev/auto-management-protocol.md`. Then run `git status --short` and `git log --oneline -10`. Then state out loud: current phase, branch, objective, approved scope, hard exclusions, what is done, what remains, and your exact next action. **Next recommended action: await Coordinator authorization for (A) design-system tokenization / component contracts or (B) the WhatsApp iOS data-foundation package — NOT raw UI implementation.** **No implementation, scaffold, dependency install, `package.json`, app-code change (`index.html` / `src/**`), or fixture import is authorized.** Do not edit any file until you have completed the reads above. Do not commit or push without explicit instruction. If `AI_HANDOFF.md` is missing, stale, or conflicts with git state, stop and ask the Coordinator.
...
| Field | Value |
|---|---|
| Phase | **Phase 0 — foundation rebuild planning; KeepMees Design Bible v1 APPROVED (2026-06-16)** (analytics-package series CLOSED/COMPLETE through Package 3AM; pivoted after the read-only dogfood audit of the `index.html` MVP) |
| Active package | None |
| Active pass | None |
| Branch | `main` |
| Expected HEAD | `756df7b` (pre-sync base; this Design Bible v1 docs checkpoint adds one docs commit on top) |
...
| Read first | `docs/design/keepmees-design-bible.md` · `docs/design/taste-trial-result.md` · `docs/architecture/phase-0-rebuild-decisions.md` |
| Next action | Await Coordinator authorization for **(A)** design-system tokenization / component contracts or **(B)** the WhatsApp iOS data-foundation package. **NOT raw UI implementation.** |
