...
> You are resuming work on **KeepMees / MessageVault**. The project is in **Phase 0 foundation rebuild planning**. The analytics-package series (through **Package 3AM**) is **CLOSED/COMPLETE and merged to `main`**; a read-only dogfood audit of the `index.html` MVP surfaced foundational defects (one-sided WhatsApp rendering, no media/ZIP intake, no real group-chat support, iMessage `attributedBody` message loss, stopword-only word analytics, copy/pluralization defects, a single iMessage-approximation renderer for all platforms), so the project has **pivoted from patching the MVP to rebuilding on a credible foundation**. **No active package. No active pass.** Branch: `main`. Expected HEAD: `f8651a8` (pre-sync base; the Phase 0 next-session-prompt sync adds one docs commit on top). **Locked Phase 0 decisions:** original premium KeepMees rendering language with **no native-app trade-dress cloning**; **AI design stream, no professional human designer**; client-side **Vite + React + TypeScript SPA via a staged strangler-fig migration**; **local-first / in-browser processing preserved** (no backend, nothing leaves the device at runtime); **WhatsApp iOS is the first proof vertical**; platform priority **WhatsApp iOS → iMessage → Meta → Telegram, Android deferred**; **design source-of-truth tool pending a Coordinator-led taste trial**. **Do not trust any memory of prior conversation.** Read, in this order: `AGENTS.md`, `CLAUDE.md` (if you are Claude Code), `AI_HANDOFF.md`, `CURRENT_STATE.md`, `docs/architecture/phase-0-rebuild-decisions.md`, `docs/design/taste-trial-brief.md`, `docs/ai-system/README.md`, `docs/dev/auto-management-protocol.md`. Then run `git status --short` and `git log --oneline -10`. Then state out loud: current phase, branch, objective, approved scope, hard exclusions, what is done, what remains, and your exact next action. **Next recommended action: produce / execute the Design Tool Taste-Trial Operator Packet — NOT implementation.** **No implementation, scaffold, dependency install, `package.json`, app-code change (`index.html` / `src/**`), or fixture import is authorized.** Do not edit any file until you have completed the reads above. Do not commit or push without explicit instruction. If `AI_HANDOFF.md` is missing, stale, or conflicts with git state, stop and ask the Coordinator.
...
| Field | Value |
|---|---|
| Phase | **Phase 0 — foundation rebuild planning** (analytics-package series CLOSED/COMPLETE through Package 3AM; pivoted after the read-only dogfood audit of the `index.html` MVP) |
| Active package | None |
| Active pass | None |
| Branch | `main` |
| Expected HEAD | `f8651a8` (pre-sync base; this Phase 0 next-session-prompt sync adds one docs commit on top) |
| Locked Phase 0 decisions | Original premium KeepMees rendering language, **no native-app trade-dress cloning** · AI design stream, **no professional human designer** · **Vite + React + TypeScript SPA** via staged strangler-fig migration · **local-first / in-browser processing preserved** (no backend) · **WhatsApp iOS first** proof vertical · platform priority **WhatsApp iOS → iMessage → Meta → Telegram, Android deferred** · **design source-of-truth tool pending taste trial** |
| Read first | `docs/architecture/phase-0-rebuild-decisions.md` · `docs/design/taste-trial-brief.md` |
| Next action | Produce / execute the **Design Tool Taste-Trial Operator Packet** (Figma vs Subframe vs Onlook, synthetic content, Coordinator-led). **Not implementation.** |
